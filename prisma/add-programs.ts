...
// ---------- تنظيف قوالب الدعوة القديمة ----------
// الدعوة أصبحت موديول مستقل (/invitations)، فنحذف قوالبها القديمة من المستندات.
const OBSOLETE_TEMPLATE_NAMES: string[] = [
  "دعوة سياحية (موجَّهة إلى قنصلية)",
  "دعوة سياحية — Français (قنصلية)",
  "دعوة سياحية — English (قنصلية)",
  "دعوة سياحية — عربي (قنصلية)",
];
...
async function main() {
  ...
  // حذف قوالب الدعوة القديمة (انتقلت لموديول مستقل)
  const del = await prisma.documentTemplate.deleteMany({
    where: { name: { in: OBSOLETE_TEMPLATE_NAMES } },
  });
  if (del.count) console.log(`\u2717 حُذف ${del.count} قالب دعوة قديم (انتقلت للموديول المستقل)`);
