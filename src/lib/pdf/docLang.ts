// اختيار لغة المستند من الرابط (?lang=ar|fr) مع لغة افتراضية لكل نوع مستند.
export type Lang = "ar" | "fr";

export function pickLang(url: string, def: Lang): Lang {
  const v = new URL(url).searchParams.get("lang");
  return v === "fr" || v === "ar" ? v : def;
}

// اتجاه الكتابة حسب اللغة (العربية من اليمين، الفرنسية من اليسار)
